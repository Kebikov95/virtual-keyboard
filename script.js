...
function init() {
    ...
    about.id = 'about';
    about.innerText = 'Клавиатура разработана в ОС Windows.\n Переключение языка реализовано комбинацией клавишь "ALT" + "Ctrl".';
...
function initKeyboard(keyboardLayout) {
    ...
    let keyboard = keyboardLayout === 'en' ? EN_KEYBOARD : RU_KEYBOARD;
        ...
        for(let i = 0; i < CODE_KEYBOARD.length; i++){
            if(code === CODE_KEYBOARD[i]){
                if(LANGUAGE.value == 'en') {
                    ...
                    if(CAPSLOCK.value) {
                        TEXTAREA.append(RU_KEYBOARD[i].toUpperCase());
                        console.log('RU', RU_KEYBOARD[i].toUpperCase());
                    } else {
                        TEXTAREA.append(RU_KEYBOARD[i]);
                        console.log('ru', RU_KEYBOARD[i]);
                    }
                }
            }
        }
